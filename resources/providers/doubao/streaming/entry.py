...
DEFAULT_URL = "wss://ai-gateway.vei.volces.com/v1/realtime"
    ...
    def recv_json(self) -> Optional[Dict[str, Any]]:
        ...
        while True:
            ...
            try:
                ...
                raise RuntimeError(f"Invalid JSON message from Doubao: {exc}") from exc
...
def build_session_update_event(model: str) -> Dict[str, Any]:
    ...
    result_type = os.getenv("VINPUT_ASR_RESULT_TYPE")
    if result_type and result_type.strip():
        session["result_type"] = int(result_type.strip())
    ...
    if get_optional_bool_env("VINPUT_ASR_ENABLE_VAD", False):
        session["turn_detection"] = {
            ...
            "silence_duration_ms": get_optional_int_env(
                "VINPUT_ASR_VAD_SILENCE_DURATION_MS", 300
            ),
        }
...
def handle_server_message(message: Dict[str, Any], state: Dict[str, Any]) -> None:
    ...
    if message_type == "transcription_session.updated":
    ...
    if message_type == "error":
        ...
        error_message = "Unknown Doubao error."
...
def run() -> int:
    ...
    client.send_json(build_session_update_event(model))
